...
public interface BillRepository extends JpaRepository<Bill, Long>, JpaSpecificationExecutor<Bill> {
    ...
    @Query(value = "select * from bill b where b.return_status='false' and DATEDIFF(DAY, b.create_date, GETDATE()) <= 7 and b.status='HOAN_THANH'", nativeQuery = true)
    Page<Bill> findValidBillToReturn(Pageable pageable);

//        @Query(value = "select * from bill b where b.return_status='false'", nativeQuery = true)
//    Page<Bill> findValidBillToReturn(Pageable pageable);

    @Query("SELECT SUM(bd.quantity * bd.momentPrice) - sum(bd.returnQuantity * bd.momentPrice) + sum(rd.quantityReturn * rd.productDetail.price) FROM Bill b join BillReturn br on b.id = br.id join ReturnDetail rd on br.id = rd.billReturn.id join BillDetail bd on b.id = bd.bill.id")
